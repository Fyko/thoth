...
pub async fn triggers(
    ...
    #[description = "The word to search triggers for (eg: cow)."] word: String,
    ...
    let words = fetch_datamuse(DatamuseQuery::Triggers, &word)
        .await?
        .into_iter()
        .map(|w| w.word)
        .collect::<Vec<_>>();
    ...
    let content = formatdoc! {"
		I found `{found_count}` words are often associated with `{word}`:

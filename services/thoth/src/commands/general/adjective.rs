...
pub async fn adjective(
    ctx: &SlashContext<ThothState>,
    #[description = "The noun to search relavent adjectives for (eg: ocean)."] word: String,
    ...
    let words = fetch_datamuse(DatamuseQuery::Adjective, &word)
        .await?
        .into_iter()
        .map(|w| w.word)
        .collect::<Vec<_>>();
    ...
    let content = formatdoc! {"
		I found `{found_count}` adjectives to describe `{word}`:

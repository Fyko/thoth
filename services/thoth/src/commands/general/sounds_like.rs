...
pub async fn sounds_like(
    ctx: &SlashContext<ThothState>,
    #[description = "The word to search similarly sounding words for (eg: tea)."] word: String,
    ...
    let words = fetch_datamuse(DatamuseQuery::SoundsLike, &word)
        .await?
        .into_iter()
        .map(|w| w.word)
        .collect::<Vec<_>>();
    ...
    let content = formatdoc! {"
		I found `{found_count}` words that sounds like `{word}`:

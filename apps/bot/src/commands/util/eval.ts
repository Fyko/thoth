...
export default class<Cmd extends typeof EvalCommand> extends Command<Cmd> {
	...
	private _clean(text: string): any {
		...
		return replacedText.replaceAll(
			new RegExp(process.env.DISCORD_TOKEN!, 'gi'),
			SENSITIVE_PATTERN_REPLACEMENT
		);
	}
	...
	public override async chatInput(
		...
		try {
			...
			// biome-ignore lint/security/noGlobalEval: eval is used for development purposes
			...
			response += MESSAGES.COMMANDS.EVAL.OUTPUT(
				this._clean(inspect(evaled, { depth: 0 }))
			);
			...
			await interaction.editReply(
				MESSAGES.COMMANDS.EVAL.ERRORS.CODE_BLOCK(
					this._clean((err.stack ?? err).toString())
				)
			);
		}
	}

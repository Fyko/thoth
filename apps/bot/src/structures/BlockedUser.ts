...
export class BlockedUserModule {
	...
	public async add(user_id: Snowflake, reason: string) {
		await this
			.sql`insert into public.thoth_bans (user_id, reason) values (${user_id}, ${reason})`;
	...
	public async remove(user_id: Snowflake) {
		await this
			.sql`delete from public.thoth_bans where user_id = ${user_id}`;
		this.bans.delete(user_id);
	}

	private async refresh() {
		const user_ids = await this.sql<
			{ reason: Reason; user_id: Snowflake }[]
		>`
			select user_id, reason from public.thoth_bans;
		`;

		this.bans = new Map(
			user_ids.map(({ user_id, reason }) => [user_id, reason])
		);
	}
